...
function layer:__init(opt)
  ...
  self.lstm = LSTM_Armour.lstm(self.rnn_size, self.rnn_size, self.num_layers, dropout)
  -- output attention model
  -- inputs: {h_t, As} 
  -- outputs: {logSoft}
  self.output_attention_model = LSTM_Armour.Make_Output_Attention_Bilinear_Unit(self.rnn_size, self.word_encoding_size, self.vocab_size + 1) 
...
function layer:createClones() 
  ...
  self.lstm_clones = {self.lstm}
  self.output_attention_model_clones = {self.output_attention_model}
  ...
  for t=2,self.seq_length+1 do  -- we will input the START Token and image at the same time
    ...
    self.lstm_clones[t] = self.lstm:clone('weight', 'bias', 'gradWeight', 'gradBias')
    self.output_attention_model_clones[t] = self.output_attention_model:clone('weight', 'bias', 'gradWeight', 'gradBias') 
...
function layer:getModulesList()
  return {self.linear_model, self.lookup_table, self.input_attention_model, self.lstm, self.output_attention_model}
end
...
function layer:parameters()
  ...
  local p4,g4 = self.lstm:parameters()
  local p5,g5 = self.output_attention_model:parameters()  
...
function layer:training()
  ...
  for k,v in pairs(self.output_attention_model_clones) do v:training() end
...
function layer:evaluate()
  ...
  for k,v in pairs(self.output_attention_model_clones) do v:evaluate() end
...
function layer:sample(input, opt)
  ...
  for t=1,self.seq_length+1 do
    ...
    if t ==1 then 
        -- out: {next_c, next_h}
        local out = self.lstm:forward(self.inputs[t][1], unpack(state))  
        -- forward to output_attetion_model 
        logprobs = self.output_attention_model:forward({out[2], self.As}) 

        state = {} 
        for i = 1, self.num_state do table.insert(state, out[i]) end         
    else -- t > 1  
        -- forward to input_attention_model 
        local g = unpack(self.input_attention_model:forward(xt)) 
        -- forward to lstm
        local out = self.lstm:forward(g, unpack(state)) 
...
function layer:updateOutput(input)
  ...
  for t=1,self.seq_length+1 do
    ...
    if not can_skip then
      ...
      if t == 1 then 
        self.lstm_inputs[t] = {x1, unpack(self.state[t-1])}  -- bz * 8 , v1  
        ...
        self.lstm_inputs[t] = {g, unpack(self.state[t-1])}
        -- out = {next_c, next_h} 
        local out = self.lstm_clones[t]:forward(self.lstm_inputs[t])
      ...
      -- forward the next_h to output attention model 
      -- inputs: {h_t, As} 
      -- outputs: {logsoft}
      local pt = self.output_attention_model_clones[t]:forward({self.state[t][self.num_state], self.As})
...
function layer:updateGradInput(input, gradOutput) 
  ...
  for t=self.tmax,1,-1 do
    ...
    local dword_t_global = nil 
    -- concat state gradients and output vector gradients at time step t
    -- local dout = {}

    -- for k=1,#dstate[t] do table.insert(dout, dstate[t][k]) end
   
    -- backprop to self.output_attention_model_clones
    local dh_t, dAs_t1 = unpack(self.output_attention_model_clones[t]:backward({self.state[t][self.num_state], self.As}, gradOutput[t]))

    -- note here that 
    -- add to dh_t 
    dstate[t][2] = dstate[t][2] + dh_t
    -- print(dstate[t][2]:sum())

    if dAs == nil then  
        dAs = torch.Tensor():typeAs(dAs_t1):resizeAs(dAs_t1):copy(dAs_t1)
    else 
        dAs = dAs + dAs_t1
    end
    ...
    if t == 1 then 
        ...
        local dlstm_inputs = self.lstm_clones[t]:backward(self.lstm_inputs[t], dstate[t])
        local dimgs_t, dc_prev, dh_prev = dlstm_inputs[1], dlstm_inputs[2], dlstm_inputs[3]

        dstate[t-1] = {} 
        table.insert(dstate[t-1], dc_prev) 
        table.insert(dstate[t-1], dh_prev) 
        -- input[1] is imgs
        dimgs = self.linear_models[t]: backward(input[1], dimgs_t) 
    else

        local lstm_out = self.lstm_clones[t]:backward(self.lstm_inputs[t], dstate[t])
       
        local dg_t, dc_prev, dh_prev = lstm_out[1], lstm_out[2], lstm_out[3]
    ...
    if t > 1 then 
        ...
        self.lookup_tables[t]:backward(it, dword_t_global) -- backprop into lookup table
    end 
  end
