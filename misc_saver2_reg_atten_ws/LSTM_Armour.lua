...
require 'misc_saver2_reg_atten_ws.LinearTensorD3'
require 'misc_saver2_reg_atten_ws.BilinearD3_version2'
require 'misc_saver2_reg_atten_ws.CAddTableD2D3'
require 'misc_saver2_reg_atten_ws.CustomAlphaView' 

require 'misc_saver2_reg_atten_ws.probe' -- for debugger on nngraph module, put the layer to check gradient and outputs 
require 'misc_saver2_reg_atten_ws.utils_bg' -- also for debugger purpose 
...
function LSTM_Armour.lstm_with_output_attention(word_embed_size, input_size, rnn_size, outputSize, n, dropout) 
  ...
  -- log_soft_beta
  local log_soft_beta = LSTM_Armour.Make_Output_Attention_Bilinear_Unit(rnn_size, word_embed_size, outputSize)({top_h, inputs[2]}) 
  -- log_soft_beta will be a nngraph.Node
  -- must use split method here to split log_soft_beta into two nodes
  local log_soft, beta = log_soft_beta:split(2)

  table.insert(outputs, log_soft) 
  table.insert(outputs, beta)

  -- outputs: next_c, next_h, logsoft, beta 
  return nn.gModule(inputs, outputs) 
...
function LSTM_Armour.Make_Output_Attention_Bilinear_Unit(hDim, word_embed_size, outputSize, dropout) 
	dropout = dropout or 0 -- no dropout 
    ...
    -- Attention weights: beta: we need to output it so that we can do some regularization on it 
	local beta = nn.SoftMax()(attention_output) -- bz * top_attrs  
	...
	local inputs, outputs = {h_t, As}, {logsoft, beta}  
