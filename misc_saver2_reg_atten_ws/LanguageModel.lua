...
local LSTM_Armour = require 'misc_saver2_reg_atten_ws.LSTM_Armour'
require 'misc_saver2_reg_atten_ws.LookupTableMaskZero'

local peek = require 'misc_saver2_reg_atten_ws.peek'
...
function layer:sample(input, opt)
  ...
  if sample_max == 1 and beam_size > 1 then return self:sample_beam(input, opt) end -- indirection for beam search
...
function layer:sample_beam(input, opt)
  ...
  for k=1,batch_size do
    ...
    -- expand out Ak
    local Ak = self.As[{{k, k},{}, {}}]:expand(beam_size, self.As:size(2), self.As:size(3)) 
    ...
    for t=1,self.seq_length+1 do
      ...
      if t == 1 then
        ...
        x1 = self.linear_model:forward(imgk)
      else -- t > 1
        --[[
      ...
      if t == 1 then 
        -- out: {next_c, next_h, logsoft}
        local inputs = {x1, Ak, unpack(state)}
        local out = self.lstm_att:forward(inputs)  
        -- forward to output_attetion_model 
        logprobs = out[self.num_state + 1]
        state = {} 
        ...
        local g = self.input_attention_model:forward(xt)
        -- forward to lstm
        local inputs = {g, Ak, unpack(state)} 
        local out = self.lstm_att:forward(inputs) 
        -- forward to output attention
        logprobs = out[self.num_state + 1]
        state = {} 
...
function layer:updateOutput(input)
  ...
  -- ***********************************************************
  -- size: bz x (self.seq_length+1) x top_attrs
  self.att_weights = self.output.new():resize(batch_size, self.seq_length+1, attrs:size(2)):zero()  
  -- ************************************************************
  self:_createInitState(batch_size)
  ...
  for t=1,self.seq_length+1 do
    ...
    if not can_skip then
      ...
      if t == 1 then 
        ...
        -- we need to take out beta from the lstm beacause we want to 
        -- do reguarition on the attention weights beta 
        self.att_weights[{{}, {t, t}, {}}] = out[self.num_state+2]
  ...
  -- after the above loop, we have stored the output attention weights matrix beta in self.att_weights 
  -- self.att_weights: bz x L x self.top_attrs
  return self.output, self.att_weights
end

--[[
gradOutput is an (D+1)xNx(M+1) Tensor.
--]]
function layer:updateGradInput(input, gradOutputTable) 
  local dimgs = nil  -- grad on input images
  local dAs = nil -- grad on the attrs 

  local gradOutput = gradOutputTable[1] -- gradOutput from language model criterion 
  local gradAttention_Output = gradOutputTable[2]  -- gradOutput from attention weights criterion 
  ...
  for t=self.tmax,1,-1 do
    ...
    -- add gradAttention_Output[t] to the  output attention model  
    -- add gradients on attention weights to dAttention_Weights 
    table.insert(dout, gradAttention_Output[{{}, {t, t}, {}}])  
